import numpy as np
from qbounds import UtilityFunction

class QBO(object):
    def __init__(self, reward_func, pbounds, beta_t=None, random_features=None, bandits=False, domain=None):
        self.linear_bandit = bandits
        self.domain = domain
    ...
    def warmup(self, init_points):
        ...
        for x in self.init_points:
            y, f_value, num_oracle_queries = self.reward_func(x, 1, self.random_features["obs_noise"])
    ...
    def acq_max(self, ac, M, random_features, bounds, nu_t, Sigma_t_inv, beta, domain, linear_bandit):
    ...
    def maximize(self, n_iter=1000, init_points=1):
        ...
        x_max = self.acq_max(ac=self.util_ucb.utility, M=M_target, random_features=self.random_features, \
        ...
        while self.total_used_queries < n_iter:
            y, f_value, num_oracle_queries = self.reward_func(x_max, self.eps_list[-1], obs_noise)
            ...
            x_max_param = self.X[self.Y.argmax()]
